...
public class BookingController {
    ...
    public ResponseEntity<List<BookingResponse>> getCustomerBookings() {
        return ResponseEntity.ok(service.getCustomerBookings());
    }

    @GetMapping("/provider")
    @PreAuthorize("hasRole('SERVICE_PROVIDER')")
    public ResponseEntity<List<BookingResponse>> getProviderBookings() {
        return ResponseEntity.ok(service.getProviderBookings());
    }

    @PatchMapping("/{id}/accept")
    @PreAuthorize("hasRole('SERVICE_PROVIDER')")
    public ResponseEntity<BookingResponse> acceptBooking(@PathVariable UUID id) {
        return ResponseEntity.ok(service.acceptBooking(id));
    }
    ...
    public ResponseEntity<BookingResponse> completeBooking(@PathVariable UUID id) {
        return ResponseEntity.ok(service.completeBooking(id));
    }
